...
def save_format_data():
    data_list = []
    try:
        for num, i in enumerate(author_coll.find({"lanv_status": 1},no_cursor_timeout=True)):
        ...
        # for num, i in enumerate(author_coll.find({'verified': True, 'status': 'no_used', 'contact_status': 2, 'lanv_status': 1}, no_cursor_timeout=True)):
            ...
            msg = {
                ...
                'verified_company': i.get('verifiedDetail').get('description'),
                'blueVType': i.get('verifiedDetail').get('type'),
